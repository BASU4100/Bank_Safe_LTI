...
public class TransactionController {
    ...
    public ResponseEntity<Integer> addTransaction(@RequestBody Transactions transaction) throws SQLException {
        if (transaction.getAccounts()==null) {
            throw new AccountNotFoundException("No account Associated with the transaction");
        }
        transaction.setAccounts(accountServiceImplJpa.getAccountById(transaction.getAccounts().getAccountId()));
